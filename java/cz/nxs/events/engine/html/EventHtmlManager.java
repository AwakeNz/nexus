...
import cz.nxs.events.engine.html.IHtmlManager;
...
implements IHtmlManager {
    private final Map<String, String> _cache = new FastMap();
    public static int BUFFS_PER_PAGE = -1;
    public static String BBS_COMMAND = null;

    public EventHtmlManager() {
        this.loadAdminHtmls();
        NexusLoader.debug((String)"Nexus Events: Loaded HTML manager.");
    }
    ...
    public void loadAdminHtmls() {
        ...
        NexusLoader.debug((String)"Nexus Events: Loading Admin htmls from the jar file.", (Level)Level.INFO);
        ZipFile jarFile = null;
        try {
            jarFile = new JarFile(NexusLoader.getLibsFolderName() + "nexus_events.jar");
            InputStream in = null;
            ...
            while (entries.hasMoreElements()) {
                try {
                    ...
                    if (!entry.getName().endsWith(".htm") && !entry.getName().endsWith(".html")) continue;
        ...
        NexusLoader.debug((String)("Loaded " + this._cache.size() + " HTML files from nexus_events.jar"), (Level)Level.INFO);
    }
    ...
    public boolean showNpcHtml(PlayerEventInfo player, NpcData npc) {
        if (npc.getNpcId() == EventConfig.getInstance().getGlobalConfigInt("miniEventsManagerId")) {
            this.showMiniEventsMenu(player);
            return true;
        }
        if (npc.getNpcId() == EventConfig.getInstance().getGlobalConfigInt("mainEventManagerId")) {
            this.showMainEventsMenu(player, npc);
            return true;
        }
        for (Map<Integer, MiniEventManager> coll : EventManager.getInstance().getMiniEvents().values()) {
            for (MiniEventManager mgr : coll.values()) {
                if (mgr.getMode() == null || mgr.getMode().getNpcId() != npc.getNpcId()) continue;
                this.showMiniEventMenu(player, mgr.getEventType(), mgr.getMode().getModeId(), false, null, "nxs_mini_view " + mgr.getMode().getModeId() + " " + mgr.getEventType().getAltTitle());
                return true;
            }
        }
        return false;
    ...
    public boolean onCbBypass(PlayerEventInfo player, String bypass) {
        block14 : {
            ...
            try {
                if (!bypass.startsWith(BBS_COMMAND)) break block14;
                ...
                if (st.hasMoreTokens()) {
                    ...
                    if (command.startsWith("showplayers_page")) {
                        ...
                        this.showHomePage(player, page, null, st.hasMoreTokens() && st.nextToken().equals("desc"));
                        return true;
                    }
                    if (command.startsWith("showinstance")) {
                        ...
                        this.showHomePage(player, instance, null, false);
                        return true;
                    }
                    if (!command.startsWith("nextpageteam")) break block14;
                    int page = 1;
                    int instance = -1;
                    try {
                        page = Integer.parseInt(st.nextToken());
                    ...
                    this.showHomePage(player, instance > 0 ? Integer.valueOf(instance) : null, page, false);
                    return true;
                }
                return this.showHomePage(player, null, null, false);
            }
            catch (Exception e) {
                if (BBS_COMMAND != null) break block14;
                BBS_COMMAND = EventConfig.getInstance().getGlobalConfigValue("cbPage");
            }
        }
        return false;
        ...
        if (EventManager.getInstance().getCurrentMainEvent() != null) {
            switch (EventManager.getInstance().getMainEventManager().getState()) {
                case REGISTERING: {
                    ...
                    if (text == null) {
                        NexusLoader.debug((String)"Missing cb_registering.htm - html file for community board in mods/nexus/ folder.", (Level)Level.WARNING);
                        return false;
                    }
                    ...
                    RegisteredPlayerList playerList = this.getRegisteredPlayerList(param != null && param instanceof Integer ? (Integer)param : 1);
                    tb.append("<br><img src=\"L2UI.SquareGray\" width=512 height=2>");
                    tb.append("<img src=\"L2UI.SquareBlank\" width=510 height=3>");
                    int maxPages = this.getMaxPages();
                    tb.append("<center><table bgcolor=2E2E2E width=510><tr><td width=510 align=center><font color=ac9887>Registered players</font> " + (page > 1 ? new StringBuilder().append("<font color=5f5f5f>(page ").append(page).append("/").append(maxPages).append(")</font>").toString() : "") + "</td></tr></table></center><br1>");
                ...
                case TELE_BACK: {
                    ...
                    if (text == null) {
                        NexusLoader.debug((String)"Missing cb_eventactive.htm - html file for community board in mods/nexus/ folder.", (Level)Level.WARNING);
                        return false;
                    }
                    AbstractMainEvent event = EventManager.getInstance().getCurrentMainEvent();
                    TextBuilder tb = new TextBuilder();
                    ...
                    tb.append("<img src=\"L2UI.SquareGray\" width=512 height=2>");
                    tb.append("<img src=\"L2UI.SquareBlank\" width=510 height=3>");
                    boolean bl = moreInstances = event.getInstances().length > 1;
                    ...
                    if (moreInstances) {
                        ...
                        for (int i = 0; i < Math.min(event.getInstances().length, 4); ++i) {
                            instances = instances + "<td align=center width=" + 510 / Math.min(event.getInstances().length, 4) + "><a action=\"bypass -h " + BBS_COMMAND + " showinstance " + i + "\"><font color=" + (event.getInstances()[i].getId() == shownInstance.getId() ? "BB9777" : "9f9f9f") + ">[Instance " + event.getInstances()[i].getName() + "]</font></a></td>, ";
                        }
            ...
            if (text == null) {
                NexusLoader.debug((String)"Missing cb_noevent.htm - html file for community board in mods/nexus/ folder.", (Level)Level.WARNING);
                return false;
            }
    ...
    protected int getMaxPages() {
        ...
        return (int)Math.ceil((double)EventManager.getInstance().getMainEventManager().getPlayers().size() / 15.0);
    }
    ...
    public boolean onBypass(PlayerEventInfo player, String bypass) {
        if (bypass.startsWith("mini_")) {
            ...
            if (action.startsWith("register")) {
                ...
                if (event.isRegularEvent()) return true;
            ...
            if (action.startsWith("unregister")) {
                ...
                int modeId = Integer.parseInt(st.nextToken());
                EventType event = EventType.getType(st.nextToken());
                if (event.isRegularEvent() || !EventManager.getInstance().getMiniEvent(event, modeId).unregisterTeam(player)) return true;
                this.showWindowUnregistered(player);
                return true;
            }
            if (action.startsWith("view")) {
                ...
                if (event.isRegularEvent()) return true;
                this.showMiniEventMenu(player, event, modeId, false, null, returnPage);
                return true;
            }
            if (action.startsWith("expand_eventinfo")) {
                ...
                if (st.hasMoreTokens()) {
                    ...
                    this.showMiniEventMenu(player, event, modeId, true, null, returnPage);
                    return true;
                }
            ...
            if (action.startsWith("showrewards")) {
                ...
                if (st.hasMoreTokens()) {
                    ...
                    this.showMiniEventMenu(player, event, modeId, false, pos, "nxs_mini_menu");
                    return true;
                }
                player.sendMessage("Wrong event.");
                return true;
            ...
            if (action.startsWith("matches_menu")) {
                ...
                if (event.isRegularEvent()) return true;
                this.showSpectateableGames(player, event, modeId, returnPage);
                return true;
            }
            ...
            if (!action.startsWith("maps")) return false;
            StringTokenizer st = new StringTokenizer(action);
            st.nextToken();
            int modeId = Integer.parseInt(st.nextToken());
            EventType event = EventType.getType(st.nextToken());
            ...
            if (event.isRegularEvent()) return true;
            this.showEventMaps(player, event, modeId, returnPage);
            return true;
        }
        if (bypass.startsWith("main_")) {
            String action = bypass.substring(5);
            if (action.startsWith("register")) {
            ...
            if (action.startsWith("unregister")) {
                ...
                this.showMainEventsMenu(player, null);
                return true;
            }
            if (!action.startsWith("menu")) return false;
            this.showMainEventsMenu(player, null);
            return true;
        }
        if (bypass.startsWith("buffer")) {
            ...
            if (action.startsWith("menu")) {
                ...
                this.showBufferHome(player, returnPage, page, null);
                return true;
            }
            ...
            if (action.startsWith("select_eventscheme_menu")) {
                ...
                this.showSelectSchemeForEventWindow(player, returnPage, event);
                return true;
            }
            if (action.startsWith("select_eventscheme_pet_menu")) {
                ...
                String returnPage = st.nextToken();
                String event = st.nextToken();
                this.showSelectSchemeForPetInEventWindow(player, returnPage, event);
                return true;
            }
            if (action.startsWith("select_scheme")) {
                ...
                this.showBufferHome(player, returnPage, 0, null);
                return true;
            }
            if (action.startsWith("edit_eventscheme")) {
                ...
                EventBuffer.getInstance().setPlayersCurrentScheme(player.getPlayersId(), scheme);
                this.showEditEventScheme(player, returnPage, menuReturnPage, page, null, event);
                return true;
            }
            ...
            if (action.startsWith("select_category")) {
                ...
                this.showBufferHome(player, returnPage, page, category);
                return true;
            }
            if (action.startsWith("addskill")) {
                ...
                this.addBuffToTemplate(player, buffId);
                this.showBufferHome(player, returnPage, page, category);
                return true;
            }
            if (action.startsWith("removeskill")) {
                ...
                int page = Integer.parseInt(st.nextToken());
                int buffId = Integer.parseInt(st.nextToken());
                this.removeBuffFromTemplate(player, buffId);
                this.showBufferHome(player, returnPage, page, category);
                return true;
            }
            if (action.startsWith("eventscheme_select_category")) {
                ...
                this.showEditEventScheme(player, returnPage, menuReturnPage, page, category, event);
                return true;
            }
            ...
            if (!action.startsWith("eventscheme_removeskill")) return false;
            StringTokenizer st = new StringTokenizer(action);
            st.nextToken();
            String returnPage = st.nextToken();
            String menuReturnPage = st.nextToken();
        ...
        if (bypass.startsWith("engine_info")) {
            ...
            this.showEngineInfoPage(player, returnPage);
            return true;
        }
        if (bypass.startsWith("features_menu")) {
            ...
            this.showEngineFeatures(player, returnPage);
            return true;
        }
        if (bypass.startsWith("statistics_menu")) {
            ...
            this.showStatisticsMenu(player, returnPage);
            return true;
        }
        if (bypass.startsWith("warnings_info")) {
            ...
            this.showWarningInfo(player, returnPage);
            return true;
        }
        if (bypass.startsWith("showrewards")) {
            ...
            if (st.hasMoreTokens()) {
                ...
                if (EventManager.getInstance().getMainEventManager().getCurrent() == null || EventManager.getInstance().getMainEventManager().getCurrent().getEventType() != event) {
                    ...
                    this.showMainEventsMenu(player, null, pos, false, false);
                }
                return true;
            }
            player.sendMessage("Wrong event.");
        ...
        if (bypass.startsWith("mapinfo")) {
            ...
            if (st.hasMoreTokens()) {
                ...
                player.sendMessage("================================");
                this.showMainEventsMenu(player, null, null, false, true);
                return true;
            } else {
        ...
        if (!bypass.startsWith("expand_eventinfo")) return false;
        this.showMainEventsMenu(player, null, null, true, false);
        return false;
    }
    ...
    public void showSelectSchemeForPetInEventWindow(PlayerEventInfo player, String returnPage, String event) {
        ...
        if (!EventBuffer.getInstance().getSchemes(player).isEmpty()) {
            ...
            for (Map.Entry<String, List<Integer>> schemes : EventBuffer.getInstance().getSchemes(player)) {
                try {
                    chosen = false;
    ...
    protected void showEditEventScheme(PlayerEventInfo player, String returnPage, String menuReturnPage, int page, String selectedCategory, String event) {
        ...
        String scheme = EventBuffer.getInstance().getPlayersCurrentScheme(player.getPlayersId());
        if (scheme == null) {
            this.showSelectSchemeMenu(player, returnPage, page, selectedCategory);
            return;
        }
        TextBuilder tb = new TextBuilder();
        tb.append("<html><title>Event Buffer</title><body>");
        ...
        tb.append("<table width=281 bgcolor=2E2E2E");
        i = 0;
        for (Map.Entry<String, Map<Integer, Integer>> e : EventBuffer.getInstance().getAviableBuffs().entrySet()) {
            String category2 = e.getKey();
            if (!category2.equals(selectedCategory)) continue;
            for (Map.Entry<Integer, Integer> buff : e.getValue().entrySet()) {
                int id = buff.getKey();
                int level = buff.getValue();
                ...
                icon = this.formatSkillIcon("0000", id);
                tb.append("<td width=33 align=left><img src=\"icon.skill" + icon + "\" width=32 height=32></td>");
                tb.append("<td width=95 align=left><button action=\"bypass -h nxs_buffer_eventscheme_addskill " + returnPage + " " + menuReturnPage + " " + category2 + " " + page + " " + id + " " + event + "\" value=\"" + name + "\" width=95 height=32 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>");
                if (++i != 2) continue;
                tb.append("</tr>");
                i = 0;
            }
        ...
        tb.append("<img src=\"L2UI.SquareGray\" width=280 height=1>");
        tb.append("<img src=\"L2UI.SquareBlank\" width=270 height=3>");
        int maxBuffs = this.getMaxBuffs(player) + Math.max(0, this.getMaxDances(player));
        ...
        i = 0;
        int counter = 0;
        Iterator<Integer> i$ = EventBuffer.getInstance().getBuffs(player).iterator();
        while (i$.hasNext()) {
            int buffId = i$.next();
            ...
            icon = this.formatSkillIcon("0000", buffId);
            name = this.trimName(name);
            tb.append("<td width=33 align=left><img src=\"icon.skill" + icon + "\" width=32 height=32></td>");
        ...
        if (maxPages > 1) {
            ...
            int width = (int)(220.0 / (double)maxPages);
    ...
    protected void showBufferHome(PlayerEventInfo player, String returnPage, int page, String selectedCategory) {
        ...
        for (Map.Entry<String, Map<Integer, Integer>> e : EventBuffer.getInstance().getAviableBuffs().entrySet()) {
            ...
            for (Map.Entry<Integer, Integer> buff : e.getValue().entrySet()) {
                ...
                icon = this.formatSkillIcon("0000", id);
                tb.append("<td width=33 align=left><img src=\"icon.skill" + icon + "\" width=32 height=32></td>");
                tb.append("<td width=95 align=left><button action=\"bypass -h nxs_buffer_addskill " + returnPage + " " + category2 + " " + page + " " + id + "\" value=\"" + name + "\" width=95 height=32 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>");
                if (++i != 2) continue;
                tb.append("</tr>");
                i = 0;
            }
    ...
    protected String trimName(String name) {
        ...
        if (split[0].equals("Chant")) {
            for (String s : split) {
                if (s.equalsIgnoreCase("Chant") || s.equals("of")) continue;
                return s;
            }
        }
    ...
    public void showMainEventsMenu(PlayerEventInfo player, NpcData npc) {
        this.showMainEventsMenu(player, npc, null, false, false);
    }
    ...
    public void showMainEventsMenu(PlayerEventInfo player, NpcData npc, String position, boolean fullDesc, boolean mapInfo) {
        ...
        if (EventManager.getInstance().getMainEventManager().getState() == MainEventManager.State.REGISTERING) ** GOTO lbl60
        if (EventManager.getInstance().getMainEventManager().getState() != MainEventManager.State.RUNNING && EventManager.getInstance().getMainEventManager().getState() != MainEventManager.State.TELE_BACK) ** GOTO lbl19
        ...
        tb.append("<br><br><button value=\"Event status\" action=\"bypass -h " + EventHtmlManager.BBS_COMMAND + "\" width=95 height=23 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></center>");
        ** GOTO lbl255
lbl19: // 1 sources:
        showNextEventTime = EventConfig.getInstance().getGlobalConfigBoolean("showNextEventTime");
        showNextEventName = EventConfig.getInstance().getGlobalConfigBoolean("showNextEventName");
        nextDelay = null;
        if (showNextEventTime && (nextDelay = EventManager.getInstance().getMainEventManager().getAutoSchedulerDelay()).equals("N/A")) {
            showNextEventTime = false;
        }
        nextEvent = null;
        if (!showNextEventName) ** GOTO lbl35
        next = EventManager.getInstance().getMainEventManager().getGuessedNextEvent();
        if (next == null) {
            showNextEventName = false;
        } else {
            nextEvent = next.getHtmlTitle();
        }
        if (!showNextEventTime) ** GOTO lbl36
        if (!showNextEventName) ** GOTO lbl35
        tb.append("<br><br><br><br><br><br><br><br><br>");
        ** GOTO lbl40
lbl35: // 2 sources:
        if (showNextEventName) ** GOTO lbl-1000
lbl36: // 2 sources:
        if (showNextEventTime) lbl-1000: // 2 sources:
        {
...
lbl40: // 3 sources:
        ...
        ** GOTO lbl255
lbl60: // 1 sources:
        currentEvent = EventManager.getInstance().getMainEventManager().getCurrent();
        tb.append("<table width=281 bgcolor=2E2E2E>");
        tb.append("<tr>");
        ...
        if (EventManager.getInstance().getMainEventManager().getPlayers().contains((Object)player)) {
        ...
        tb.append("<img src=\"L2UI.SquareGray\" width=280 height=1>");
        tb.append("<img src=\"L2UI.SquareBlank\" width=270 height=3>");
        desc = currentEvent.getHtmlDescription();
        ...
        tb.append("<table width=281 bgcolor=2E2E2E>");
        tb.append("<tr>");
        tb.append("<td width=281><font color=" + (fullDesc != false ? "898989" : "696969") + ">" + desc + "</font></td>");
        tb.append("</tr>");
        tb.append("</table>");
        tb.append("<img src=\"L2UI.SquareBlank\" width=280 height=3>");
        tb.append("<img src=\"L2UI.SquareGray\" width=280 height=1>");
        ...
        timeLeft = EventManager.getInstance().getMainEventManager().getTimeLeft(true);
        minutes = Integer.valueOf(timeLeft.split(":")[0]);
        seconds = Integer.valueOf(timeLeft.split(":")[1]);
        if (minutes > 0) {
        ...
        tb.append("<table width=281 bgcolor=2E2E2E>");
        tb.append("<tr>");
        positions = new TextBuilder();
        rewards = EventRewardSystem.getInstance().getAllRewardsFor(currentEvent.getEventType(), 1);
        pos = null;
        if (position != null) {
        ...
        for (Map.Entry<EventRewardSystem.PositionContainer, Map<Integer, EventRewardSystem.RewardItem>> e : rewards.getAllRewards().entrySet()) {
            ...
            if (e.getKey().position.posType == null) continue;
            pos = e.getKey().position;
            if (pos.posType == RewardPosition.PositionType.General) {
                if (position != null && position.equals(pos.toString()) || pos != RewardPosition.Winner && pos != RewardPosition.Looser) continue;
                positions.append("" + pos.toString() + ";");
                continue;
            }
            if (pos.posType == RewardPosition.PositionType.Numbered && pos != RewardPosition.KillingSpree) {
                if (position != null && (e.getKey().parameter + ".").equals(position)) continue;
                positions.append("" + e.getKey().parameter + ". ;");
                continue;
            }
            if (pos.posType != RewardPosition.PositionType.Range) continue;
            try {
                from = Integer.parseInt(e.getKey().parameter.split("-")[0]);
                to = Integer.parseInt(e.getKey().parameter.split("-")[1]);
                if (position != null && ("" + from + ".-" + to + ".").equals(position)) continue;
                positions.append("" + from + ". - " + to + ".;");
            }
            catch (Exception e2) {}
        }
        hasRewards = false;
        positionsString = positions.toString();
        if (positionsString.length() <= 0) ** GOTO lbl-1000
        hasRewards = true;
        ...
        if (hasRewards) {
            if (position == null) {
                ...
                posType = position.split("-").length > 1 ? RewardPosition.PositionType.Range : (position.endsWith(".") != false ? RewardPosition.PositionType.Numbered : RewardPosition.PositionType.General);
                for (Map.Entry<EventRewardSystem.PositionContainer, Map<Integer, EventRewardSystem.RewardItem>> e : rewards.getAllRewards().entrySet()) {
                    if (e.getKey().position == null || e.getKey().position.posType != posType) continue;
                    ...
                    if (posType == RewardPosition.PositionType.Numbered) {
...
lbl221: // 6 sources:
                    ...
                    bg = false;
                    name = null;
                    for (Map.Entry<Integer, EventRewardSystem.RewardItem> item : e.getValue().entrySet()) {
                        if (bg) {
                            tb.append("<table width=281 bgcolor=2E2E2E>");
                        } else {
                        ...
                        bg = bg == false;
                        tb.append("<tr>");
                        name = item.getValue().id == -1 ? "XP" : (item.getValue().id == -2 ? "SP" : (item.getValue().id == -3 ? "Fame" : CallBack.getInstance().getOut().getItemName(item.getValue().id)));
                        if (name.length() > 27) {
                        ...
                        min = null;
                        max = null;
                        min = item.getValue().minAmmount >= 1000000000 ? "" + (int)Math.round((double)item.getValue().minAmmount / 1.0E9) + "kkk" : (item.getValue().minAmmount >= 1000000 ? "" + (int)Math.round((double)item.getValue().minAmmount / 1000000.0) + "kk" : (item.getValue().minAmmount >= 1000 ? "" + (int)Math.round((double)item.getValue().minAmmount / 1000.0) + "k" : "" + item.getValue().minAmmount + "x"));
                        max = item.getValue().maxAmmount >= 1000000000 ? "" + (int)Math.round((double)item.getValue().maxAmmount / 1.0E9) + "kkk" : (item.getValue().maxAmmount >= 1000000 ? "" + (int)Math.round((double)item.getValue().maxAmmount / 1000000.0) + "kk" : (item.getValue().maxAmmount >= 1000 ? "" + (int)Math.round((double)item.getValue().maxAmmount / 1000.0) + "k" : "" + item.getValue().maxAmmount + "x"));
...
lbl255: // 6 sources:
    ...
    public void showMiniEventsMenu(PlayerEventInfo player) {
        ...
        for (EventType type : EventType.values()) {
            ...
            int countAlive = 0;
            for (Map.Entry<Integer, MiniEventManager> e2 : EventManager.getInstance().getMiniEvents().get((Object)type).entrySet()) {
                if (!e2.getValue().getMode().isAllowed() || !e2.getValue().getMode().isRunning()) continue;
                ++countAlive;
            }
            if (countAlive == 0) continue;
            if (size > 0) {
                tb.append("<img src=\"L2UI.SquareBlank\" width=280 height=3>");
                tb.append("<img src=\"L2UI.SquareGray\" width=280 height=1>");
                tb.append("<img src=\"L2UI.SquareBlank\" width=270 height=3>");
            ...
            tb.append("<img src=\"L2UI.SquareBlank\" width=270 height=3>");
            for (Map.Entry<Integer, MiniEventManager> e2 : EventManager.getInstance().getMiniEvents().get((Object)type).entrySet()) {
                if (!e2.getValue().getMode().isAllowed() || !e2.getValue().getMode().isRunning() || !e2.getValue().canRun()) continue;
                tb.append("<table width=281><tr>");
                if (player != null && player.getRegisteredMiniEvent() != null && player.getRegisteredMiniEvent().equals(e2.getValue())) {
                    tb.append("<td width=180><font color=494949>Mode</font><font color=595959> " + e2.getValue().getMode().getModeId() + "</font>  <font color=D29115>" + e2.getValue().getMode().getVisibleName() + "</font>" + "</td>");
                } else {
                    tb.append("<td width=180><font color=494949>Mode</font><font color=595959> " + e2.getValue().getMode().getModeId() + "</font>  <font color=9f9f9f>" + e2.getValue().getMode().getVisibleName() + "</font>" + "</td>");
                }
                tb.append("<td width=70><a action=\"bypass -h nxs_mini_view " + e2.getKey() + " " + type.getAltTitle() + "\"><font color=9f9f9f>View info</font></a></td>");
                tb.append("<td width=40 align=right>");
                try {
                    int count = e2.getValue().getRegisteredTeamsCount();
                    tb.append("<font color=7A7A7A>" + count + "&nbsp;&nbsp;</font>");
                }
                catch (Exception ex) {
                    NexusLoader.debug((String)("sent invalid bypass, event " + type.getAltTitle()));
    ...
    public void showMiniEventMenu(PlayerEventInfo player, EventType event, int modeId) {
        this.showMiniEventMenu(player, event, modeId, false, null, "nxs_mini_menu");
    }
    ...
    public void showMiniEventMenu(PlayerEventInfo player, EventType event, int modeId, boolean fullDesc, String position, String returnPage) {
        ...
        if ((manager = EventManager.getInstance().getMiniEvent(event, modeId)) == null) {
            player.sendStaticPacket();
            return;
        }
        tb = new TextBuilder();
        ...
        tb.append("<img src=\"L2UI.SquareGray\" width=280 height=1>");
        tb.append("<img src=\"L2UI.SquareBlank\" width=270 height=3>");
        desc = manager.getHtmlDescription();
        if (!(fullDesc || desc.length() <= 170)) {
        ...
        tb.append("</tr>");
        minLevel = manager.getInt("MinLevelToJoin");
        maxLevel = manager.getInt("MaxLevelToJoin");
        for (AbstractFeature f : manager.getMode().getFeatures()) {
            if (!(f instanceof LevelFeature)) continue;
            if (minLevel < ((LevelFeature)f).getMinLevel()) {
                minLevel = ((LevelFeature)f).getMinLevel();
            }
            if (maxLevel <= ((LevelFeature)f).getMaxLevel()) continue;
        ...
        tb.append("</tr>");
        needParty = manager.requireParty();
        partySize = manager.getDefaultPartySizeToJoin();
        tb.append("<tr>");
        ...
        if (needParty) {
            ...
            tb.append("<td width=120 align=right><font color=" + (meets != false ? "6D8053" : "805353") + ">Party of " + partySize + " people</font></td>");
        } else {
        ...
        tb.append("</table>");
        haveToWait = manager.getDelayHaveToWaitToJoinAgain(player);
        if (haveToWait > 0) {
            min = false;
            if ((haveToWait/=1000) > 60) {
                min = true;
                haveToWait/=60;
            }
            tb.append("<table width=281 bgcolor=2E2E2E");
            tb.append("<tr>");
            tb.append("<td width=281 align=center><font color=7D5555>You can register again in " + haveToWait + " " + (min != false ? "minutes" : "seconds") + ".</font></td>");
            tb.append("</tr>");
            tb.append("</table>");
        }
        tb.append("<table width=281 bgcolor=2E2E2E");
        ...
        tb.append("<tr>");
        positions = new TextBuilder();
        rewards = EventRewardSystem.getInstance().getAllRewardsFor(manager.getEventType(), modeId);
        pos = null;
        if (position != null) {
            if (position.split("-").length > 1) {
                from = position.split("-")[0];
...
lbl205: // 6 sources:
    ...
    protected void showEventMaps(PlayerEventInfo player, EventType event, int modeId, boolean returnPage) {
        ...
        for (EventMap map : EventMapSystem.getInstance().getMaps(event).values()) {
            if (manager.getMode().getDisMaps().contains((Object)map.getGlobalId()) || !manager.canRun(map)) continue;
        ...
        html = returnPage ? html.replaceAll("%returnPage%", "<button value=\"Back\" action=\"bypass -h nxs_mini_view " + modeId + " " + event.getAltTitle() + "\" width=70 height=19 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">") : html.replaceAll("%returnPage%", "");
        player.sendHtmlText(html);
    }
    ...
    protected class RegisteredPlayerList {
        ...
        protected RegisteredPlayerList() {
        }
    }
