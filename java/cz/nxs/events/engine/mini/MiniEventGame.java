...
public abstract class MiniEventGame
implements Runnable,
EventGame {
    ...
    protected void initAnnouncer() {
        ...
        this._announcer.setTime(System.currentTimeMillis() + (long)this.getGameTime());
    }
    ...
    public void scheduleLocChecker() {
        ...
        this._locChecker = CallBack.getInstance().getOut().scheduleGeneral(this._locCheckerInstance, _locCheckDelay);
    }
    ...
    public void addSpectator(PlayerEventInfo player) {
        ...
        if ((spectatorLoc = this.getMap().getNextSpawn(-1, SpawnType.Spectator)) == null) {
    ...
    public void removeSpectator(PlayerEventInfo pi, boolean disconnect) {
        ...
        this._spectators.remove((Object)pi);
    }

    protected void cleanSpectators() {
        if (this._spectators != null) {
            for (PlayerEventInfo pi : this._spectators) {
    ...
    protected void initEvent() {
        ...
        if (!this._event.getString("notAllowedSkills").equals("")) {
            splits = this._event.getString("notAllowedSkills").split(",");
        ...
        if (!this._event.getString("notAllowedItems").equals("")) {
            splits = this._event.getString("notAllowedItems").split(",");
    ...
    private void loadOverridenSkillsParameters() {
        ...
        try {
            ...
            try {
                ...
                NexusLoader.debug((String)("Error while loading GLOBAL config 'setOffensiveSkills' in event " + this._event.getEventName() + " - " + e.toString()), (Level)Level.SEVERE);
            }
        }
        catch (Exception e) {
            NexusLoader.debug((String)("Error while loading GLOBAL config 'setOffensiveSkills' in event " + this._event.getEventName() + " - " + e.toString()), (Level)Level.SEVERE);
        ...
        try {
            ...
            try {
                ...
                NexusLoader.debug((String)("Error while loading GLOBAL config 'setNotOffensiveSkills' in event " + this._event.getEventName() + " - " + e.toString()), (Level)Level.SEVERE);
            }
        }
        catch (Exception e) {
            NexusLoader.debug((String)("Error while loading GLOBAL config 'setNotOffensiveSkills' in event " + this._event.getEventName() + " - " + e.toString()), (Level)Level.SEVERE);
        ...
        try {
            ...
            try {
                ...
                NexusLoader.debug((String)("Error while loading GLOBAL config 'setNeutralSkills' in event " + this._event.getEventName() + " - " + e.toString()), (Level)Level.SEVERE);
            }
        }
        catch (Exception e) {
            NexusLoader.debug((String)("Error while loading GLOBAL config 'setNeutralSkills' in event " + this._event.getEventName() + " - " + e.toString()), (Level)Level.SEVERE);
    ...
    protected void abortDueToError(String message) {
        ...
        this.clearEvent();
        EventManager.getInstance().debug((Object)this._event.getEventType() + " match automatically aborted: " + message);
    }
    ...
    protected boolean checkTeamStatus(int teamId) {
        for (PlayerEventInfo pi : this.getTeams()[teamId - 1].getPlayers()) {
    ...
    protected void loadFences() {
        try {
            this._fences = new FastList();
            for (EventSpawn spawn : this._arena.getSpawns(0, SpawnType.Fence)) {
    ...
    private void loadMapGuards() {
        ...
        for (EventSpawn spawn : this.getMap().getSpawns()) {
            if (spawn.getSpawnType() != SpawnType.MapGuard) continue;
            try {
                NpcData data = template.doSpawn(spawn.getLoc().getX(), spawn.getLoc().getY(), spawn.getLoc().getZ(), 1, this._instanceId);
                if (this._mapGuards == null) {
                    this._mapGuards = new FastList();
                }
    ...
    protected void loadNpcs() {
        for (EventSpawn spawn : this.getMap().getSpawns(-1, SpawnType.Npc)) {
            try {
                int npcId = spawn.getNpcId();
                if (npcId == -1) continue;
                NpcData data = new NpcTemplateData(npcId).doSpawn(spawn.getLoc().getX(), spawn.getLoc().getY(), spawn.getLoc().getZ(), 1, this._instanceId);
                if (this._npcs == null) {
                    this._npcs = new FastList();
                }
                this._npcs.add(data);
            }
            catch (Exception e) {
    ...
    protected void loadBuffers() {
        try {
            ...
            for (AbstractFeature feature : this._event.getMode().getFeatures()) {
                if (feature.getType() != EventMode.FeatureType.Buffer) continue;
                if (!((BufferFeature)feature).canSpawnBuffer()) {
                    return;
                }
                if (((BufferFeature)feature).getCustomNpcBufferId() == 0) continue;
            ...
            for (EventSpawn spawn : this._arena.getSpawns()) {
                if (spawn.getSpawnType() != SpawnType.Buffer) continue;
                NpcData data = template.doSpawn(spawn.getLoc().getX(), spawn.getLoc().getY(), spawn.getLoc().getZ(), 1, this._instanceId);
                if (this._buffers == null) {
                    this._buffers = new FastList();
                }
    ...
    protected void unspawnMapGuards() {
        if (this._mapGuards != null) {
            for (NpcData npc : this._mapGuards) {
                if (npc == null) continue;
                npc.deleteMe();
            }
    ...
    protected void handleDoors(int state) {
        ...
        if (state == 0) {
            Iterator<EventSpawn> i$ = this._arena.getDoors().iterator();
            while (i$.hasNext()) {
                DoorAction action;
            ...
            for (DoorData door : CallBack.getInstance().getOut().getInstanceDoors(this._instanceId)) {
                for (EventSpawn doorSpawn : this._arena.getDoors()) {
                    ...
                    if (doorSpawn.getDoorId() != door.getDoorId()) continue;
                    if (action == DoorAction.Close && door.isOpened()) {
                        door.closeMe();
                        continue;
                    }
                    if (action != DoorAction.Open || door.isOpened()) continue;
                    door.openMe();
                }
            }
            return;
    ...
    protected void scheduleMessage(final String message, int delay, final boolean abortable) {
        CallBack.getInstance().getOut().scheduleGeneral(new Runnable(){
            ...
            @Override
            public void run() {
                MiniEventGame.this.broadcastMessage(message, abortable);
            }
        }, delay);
    }
    ...
    protected int getGameTime() {
        for (AbstractFeature f : this._event.getMode().getFeatures()) {
            if (f.getType() != EventMode.FeatureType.TimeLimit) continue;
    ...
    private void voteEnd(PlayerEventInfo player) {
        ...
        if (!this._voted.contains((Object)player)) {
            ...
            for (EventTeam t : this.getTeams()) {
                for (PlayerEventInfo p : t.getPlayers()) {
                    if (this._voted.contains((Object)p)) continue;
                    return;
                }
            }
    ...
    public boolean canUseItem(PlayerEventInfo player, ItemData item) {
        ...
        for (AbstractFeature f : this.getEvent().getMode().getFeatures()) {
            ...
            if (f.getType() != EventMode.FeatureType.Enchant || ((EnchantFeature)f).checkItem(player, item)) continue;
            return false;
        }
    ...
    public boolean canUseSkill(PlayerEventInfo player, SkillData skill) {
        ...
        for (AbstractFeature f : this.getEvent().getMode().getFeatures()) {
            if (f.getType() != EventMode.FeatureType.Skills || ((SkillsFeature)f).checkSkill(player, skill)) continue;
            return false;
        }
    ...
    implements Runnable {
        ...
        @Override
        public void run() {
            int delay = (int)Math.round((double)(this._start - System.currentTimeMillis()) / 1000.0);
