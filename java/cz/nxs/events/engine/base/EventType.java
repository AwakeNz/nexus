...
public enum EventType {
        ...
        if (this.eventClass != null) {
            try {
                if (this.isRegularEvent()) {
                    ...
                    if (constructor == null) {
                        NexusLoader.debug((String)("Wrong constructor for event " + this.getAltTitle() + "."), (Level)Level.SEVERE);
                        return null;
                    }
                    return constructor.newInstance(new Object[]{this, manager});
                }
            }
            catch (Exception e) {
    ...
    public static EventType getType(String value) {
        for (EventType t : EventType.values()) {
            if (!t.toString().equalsIgnoreCase(value) && !t.getAltTitle().equalsIgnoreCase(value) && !t.getHtmlTitle().equalsIgnoreCase(value) && !String.valueOf(t.getId()).equals(value)) continue;
            return t;
        }
        return null;
    }
    ...
    public static EventType getEventByMainId(int id) {
        for (EventType t : EventType.values()) {
            if (t.getMainEventId() != id) continue;
            return t;
        }
        return null;
    }
    ...
    public static EventType getNextRegularEvent() {
        ...
        if (t == null) {
            NexusLoader.debug((String)"getNextRegularEvent() returned null, all events are disabled (or have no maps).", (Level)Level.WARNING);
            return null;
        }
    ...
    public static EventType getNextRegularEvent(int lastId) {
        ...
        for (int i = 0; i < EventType.values().length; ++i) {
            for (EventType t : EventType.values()) {
                if (t.getId() != next || !t.isRegularEvent() || !EventConfig.getInstance().isEventAllowed(t) || EventManager.getInstance().getMainEvent(t) == null || EventMapSystem.getInstance().getMapsCount(t) <= 0) continue;
                return t;
            }
            ++next;
        }
        for (EventType t : EventType.values()) {
            if (!t.isRegularEvent() || !EventConfig.getInstance().isEventAllowed(t) || EventManager.getInstance().getMainEvent(t) == null || EventMapSystem.getInstance().getMapsCount(t) <= 0) continue;
            return t;
        }
        NexusLoader.debug((String)"getNextRegularEvent(int lastId) returned null, all events are DISABLED (or have no maps).", (Level)Level.WARNING);
        return null;
    }
