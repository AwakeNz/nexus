...
public class ConfigModel {
    ...
    public ConfigModel(String key, String value, String description, String defaultVal, InputType input, String inputParams) {
        ...
        if (this.input == InputType.Boolean) {
            this.addEnumOptions(new String[]{"True", "False"});
        } else {
    ...
    public ConfigModel addEnumOptions(String[] options) {
        ...
        NexusLoader.debug((String)("can't add enum options to a non enum config model. (config key = " + this.key + ")"));
        return this;
    }
    ...
    public void addToValue(String value) {
        if (this.input == InputType.MultiAdd) {
            ...
            NexusLoader.debug((String)("can't add MultiAdd options to a non MultiAdd config model. (config key = " + this.key + ")"));
        }
    }
    ...
    public String removeMultiAddValueIndex(int index) {
        ...
        NexusLoader.debug((String)("can't remove multiadd value by index from a non-MultiAdd config model. (config key = " + this.key + ")"));
        return this.value;
    }
    ...
    public String[] getValues() {
        ...
        NexusLoader.debug((String)("can't call getValues() method for a non-MultiAdd config model. (config key = " + this.key + ")"));
    ...
    public String getInputHtml(int width, int height) {
        switch (this.input) {
            case Enum: 
            case Boolean: {
                return "<combobox width=" + width + " height=" + (height == 0 ? 17 : height) + " var=" + this.getKey() + " list=" + (Object)this.inputParams + ">";
            }
            case MultiEdit: {
                if (height > 0) {
