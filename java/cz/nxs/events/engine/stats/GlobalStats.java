...
public class GlobalStats extends EventStats
{
	...
	public GlobalStatsModel getPlayerGlobalStatsCopy(PlayerEventInfo player, EventType type)
	{
		...
		FastMap<GlobalStatType, Integer> stats = new FastMap<>();
	...
	public void load()
	{
		_playerGlobalStats = new FastMap<>();
		_playerGlobalStatsHtml = new FastMap<>();
		loadConfigs();
		loadGlobalStats();
	...
	private void showPlayersGlobalStats(PlayerEventInfo player, PlayerEventInfo target, String sortType, String page, boolean backToCbMenu)
	{
		...
		String text = null;
		text = !_playerGlobalStatsHtml.containsKey(target) ? generatePlayersGlobalStatsHtml(target) : _playerGlobalStatsHtml.get(target);
		if ((text = addExtraData(text, sortType, page, backToCbMenu)) != null)
		{
			showHtmlText(player, text);
	...
	private String generatePlayersGlobalStatsHtml(PlayerEventInfo player)
	{
		...
		if (_showDetailedPlayerInfo && ((sum = this._data.get(player.getPlayersId())) != null))
		{
		...
		for (EventType event : EventType.values())
		{
			...
			GlobalStatsModel stats = _playerGlobalStats.get(player).get(event);
			if (stats == null)
			{
				continue;
			}
	...
	private void loadPlayer(PlayerEventInfo player)
	{
		...
		{
			Map<PlayerEventInfo, Map<EventType, GlobalStatsModel>> map = _playerGlobalStats;
			synchronized (map)
			{
				_playerGlobalStats.put(player, new FastMap<>());
			}
		}
		Connection con = null;
		try
		{
			...
			while (rset.next())
			{
				...
				FastMap<GlobalStatType, Integer> map = new FastMap<>();
				...
				GlobalStatsModel stats = new GlobalStatsModel(type, map);
				try
				{
					Map<PlayerEventInfo, Map<EventType, GlobalStatsModel>> map2 = _playerGlobalStats;
					synchronized (map2)
					{
						_playerGlobalStats.get(player).put(type, stats);
						continue;
	...
	public void updateGlobalStats(Map<PlayerEventInfo, GlobalStatsModel> data)
	{
		...
		try
		{
			...
			for (Map.Entry<PlayerEventInfo, GlobalStatsModel> e : data.entrySet())
			{
				...
				GlobalStatsModel stats = getPlayerGlobalStats(e.getKey(), e.getValue().getEvent());
				if (stats == null)
				{
					stats = e.getValue();
	...
	protected void loadGlobalStats()
	{
		...
		_globalStatsHtml = new FastMap<>();
		TextBuilder tb = new TextBuilder();
		Connection con = null;
		PreparedStatement statement = null;
		String charName = null;
		int playersPerPage = _playersPerPage;
		String condition = "";
		...
		try
		{
			...
			while (rset.next())
			{
				...
				if ((timesPlayed = rset.getInt("count_played")) > _data.get(Integer.valueOf(charId)).mostPlayedCount)
				{
					_data.get(Integer.valueOf(charId)).mostPlayedCount = timesPlayed;
					_data.get(Integer.valueOf(charId)).mostPlayedEvent = EventType.getType(rset.getString("event"));
				}
			...
			{
				if (type == 1 ? (sortType != SortType.NAME) && (sortType != SortType.LEVEL) : (type == 2) && (sortType != SortType.NAME) && (sortType != SortType.LEVEL) && (sortType != SortType.COUNTPLAYED) && (sortType != SortType.KDRATIO))
				{
					continue;
				}
				FastList<GlobalStatsSum> sorted = new FastList<>();
				...
				for (GlobalStatsSum stats : sorted)
				{
					...
					{
						_globalStatsHtml.put(sortType, new FastMap<>());
					}
	...
	private synchronized void scheduleReloadGlobalStats()
	{
		...
		_globalStatsReload = CallBack.getInstance().getOut().scheduleGeneral(() -> loadGlobalStats(), _statsRefresh * 1000);
	}
