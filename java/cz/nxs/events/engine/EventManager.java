...
public class EventManager {
    ...
    public EventManager() {
        this.compareByLevels = new Comparator<PlayerEventInfo>(){
        ...
        this.compareByPvps = new Comparator<PlayerEventInfo>(){
        ...
        this.compareTeamKills = new Comparator<EventTeam>(){
        ...
        this.comparePlayersKills = new Comparator<PlayerEventInfo>(){
        ...
        this.comparePlayersScore = new Comparator<PlayerEventInfo>(){
        ...
        this.compareTeamScore = new Comparator<EventTeam>(){
        ...
        NexusLoader.debug((String)"Nexus engine: Loading events...");
        this.loadEvents();
    }
    ...
    private void loadEvents() {
        ...
        for (EventType event : EventType.values()) {
            Event eventInstance;
            if (event == EventType.Unassigned || (eventInstance = event.loadEvent(this._mainManager)) == null || !event.isRegularEvent()) continue;
            this._mainEvents.put(eventInstance.getEventType(), (AbstractMainEvent)eventInstance);
            ++count;
        }
        NexusLoader.debug((String)("Nexus engine: Loaded " + count + " main events."));
        this._miniEvents.put(EventType.Unassigned, (Map<Integer, MiniEventManager>)new FastMap());
        this._miniEvents.put(EventType.Classic_1v1, (Map<Integer, MiniEventManager>)new FastMap());
        this._miniEvents.put(EventType.PartyvsParty, (Map<Integer, MiniEventManager>)new FastMap());
        this._miniEvents.put(EventType.Korean, (Map<Integer, MiniEventManager>)new FastMap());
        this._miniEvents.put(EventType.MiniTvT, (Map<Integer, MiniEventManager>)new FastMap());
        NexusLoader.debug((String)("Nexus engine: Loaded " + this._miniEvents.size() + " mini event types."));
    }
    ...
    public MiniEventManager createManager(EventType type, int modeId, String name, String visibleName, boolean loadConfigs) {
        ...
        switch (type) {
            ...
            default: {
                NexusLoader.debug((String)("Event " + type.getAltTitle() + " isn't implemented yet."), (Level)Level.WARNING);
                return null;
            }
        }
        ...
        this._miniEvents.get((Object)type).put(modeId, manager);
        return manager;
    }
    ...
    public MiniEventManager getMiniEvent(EventType type, int id) {
        if (this._miniEvents.get((Object)type) == null) {
            return null;
        }
        return this._miniEvents.get((Object)type).get(id);
    }

    public AbstractMainEvent getMainEvent(EventType type) {
        if (!this._mainEvents.containsKey((Object)type)) {
            return null;
        }
        return this._mainEvents.get((Object)type);
    }
    ...
    public void removeEventSkills(PlayerEventInfo player) {
        for (SkillData sk : player.getSkills()) {
            if (sk.getId() < 35000 || sk.getId() > 35099) continue;
    ...
    public void onPlayerLogin(final PlayerEventInfo player) {
        ...
        for (Map.Entry<DisconnectedPlayerData, Long> e : this._disconnectedPlayers.entrySet()) {
            if (e.getKey().player.getPlayersId() != player.getPlayersId()) continue;
            data = e.getKey();
    ...
    public void spectateGame(PlayerEventInfo player, EventType event, int modeId, int gameId) {
        ...
        for (MiniEventGame g : manager.getActiveGames()) {
            if (g.getGameId() != gameId) continue;
            game = g;
            break;
        }
        if (game == null) {
    ...
    public void debug(String message) {
        NexusLoader.debug((String)message);
    }
    ...
    public class AdminNexus
    extends AdminCommandHandlerInstance {
        private final String[] ADMIN_COMMANDS;

        public AdminNexus() {
            this.ADMIN_COMMANDS = new String[]{"admin_event_manage"};
        }
        ...
        public boolean useAdminCommand(String command, PlayerEventInfo activeChar) {
            if (command.startsWith("admin_event_manage")) {
                ...
                st.nextToken();
                if (!st.hasMoreTokens()) {
                    NexusEvents.onAdminBypass((PlayerEventInfo)activeChar, (String)"menu");
                } else {
                    NexusEvents.onAdminBypass((PlayerEventInfo)activeChar, (String)command.substring(19));
                }
            }
            return true;
        }
