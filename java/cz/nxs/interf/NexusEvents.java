...
public class NexusEvents
{
	...
	public static void start()
	{
		...
		NexusLoader.init(_branch, 2.1, "L2Jserver HighFive", true, "../libs/", "config/nexus_serial.txt", false);
	}
	...
	public static boolean canUseSkill(L2PcInstance player, Skill skill)
	{
		PlayerEventInfo pi = NexusEvents.getPlayer(player);
		if (pi != null)
		{
	...
	public static void onUseSkill(L2PcInstance player, Skill skill)
	{
		PlayerEventInfo pi = NexusEvents.getPlayer(player);
		if (pi != null)
		{
	...
	public static int allowTransformationSkill(L2PcInstance player, Skill s)
	{
		PlayerEventInfo pi = NexusEvents.getPlayer(player);
		if (pi != null)
		{
	...
	public static boolean isSkillOffensive(L2PcInstance activeChar, Skill skill)
	{
		PlayerEventInfo pi = NexusEvents.getPlayer(activeChar);
		if ((pi != null) && pi.isInEvent())
		{
			EventGame game = pi.getEvent();
			int val = game.isSkillOffensive(new SkillDataEvent(skill));
			if (val == 1)
			{
				return true;
			}
	...
	public static boolean isSkillNeutral(L2PcInstance activeChar, Skill skill)
	{
		PlayerEventInfo pi = NexusEvents.getPlayer(activeChar);
		if ((pi != null) && pi.isInEvent())
		{
			EventGame game = pi.getEvent();
			return game.isSkillNeutral(new SkillDataEvent(skill));
		}
		return false;
	}
}
