...
public class NexusOut
implements INexusOut {
    public void load() {
        CallBack.getInstance().setNexusOut((INexusOut)this);
    }
    ...
    public void registerAdminHandler(AdminCommandHandlerInstance handler) {
        AdminCommandHandler.getInstance().registerHandler((IAdminCommandHandler)handler);
    }
    ...
    public void announceToAllScreenMessage(String message, String announcer) {
        Broadcast.toAllOnlinePlayers((L2GameServerPacket)new CreatureSay(0, 18, "", announcer + ": " + message));
    }
    ...
    public void spawnFences(List<FenceData> list, int instance) {
        for (FenceData fence : list) {
            if (fence.getOwner() == null) continue;
            if (instance > 0) {
